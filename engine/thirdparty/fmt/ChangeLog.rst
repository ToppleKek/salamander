...
* Introduced the experimental ``fmt::compile`` function that does format string
  ...
  which can be beneficial when identically formatting many m_objects of the same
...
* [Breaking] ``fmt::(s)printf`` now supports formatting of m_objects of user-defined types
...
* All formatting methods are now implemented as variadic functions instead
  ...
  is no need to construct temporary formatter m_objects and control arguments'
